...
TEST_CASE
(   "generic_color default constructor zero-initializes channels"
,   "[color][ctors]"
)
...
TEST_CASE
(   "generic_color array constructor copies from a std::array"
,   "[color][ctors]"
)
...
TEST_CASE
(   "generic_color iterator constructor copies the range"
,   "[color][ctors]"
)
...
TEST_CASE
(   "generic_color copy constructor duplicates the channels"
,   "[color][ctors]"
)
...
TEST_CASE
(   "generic_color move constructor transfers the channels"
,   "[color][ctors]"
)
{   generic_color<int, 3> a{1, 2, 3};
...
TEST_CASE
(   "generic_color initializer list constructor sets the channels"
,   "[color][ctors]"
)
...
TEST_CASE
(   "generic_color::at provides bounds-checked access"
,   "[color][element_access]"
)
{   generic_color<int, 4> c{1, 2, 3, 4};
...
TEST_CASE
(   "generic_color::operator[] provides unchecked access"
,   "[color][element_access]"
)
{   generic_color<int, 4> c{1, 2, 3, 4};
    REQUIRE(c[0] == 1);
...
TEST_CASE
(   "generic_color::front and back access the first and last channels"
,   "[color][element_access]"
)
{   generic_color<int, 4> c{1, 2, 3, 4};
    REQUIRE(c.front() == 1);
...
TEST_CASE
(   "generic_color::data provides direct access to the underlying storage"
,   "[color][element_access]"
)
{   generic_color<int, 4> c{1, 2, 3, 4};
    REQUIRE(c.data() == &c[0]);
...
TEST_CASE
(   "generic_color::begin/end iterate over the channels in order"
,   "[color][iterators]"
)
{   generic_color<int, 4> c{1, 2, 3, 4};
...
TEST_CASE
(   "generic_color::cbegin/cend provide const iteration"
,   "[color][iterators]"
)
{   generic_color<int, 4> c{1, 2, 3, 4};
...
TEST_CASE
(   "generic_color range-based for loop works with begin/end"
,   "[color][iterators]"
)
{   generic_color<int, 4> c{1, 2, 3, 4};
    int sum = 0;
...
TEST_CASE
(   "generic_color::rbegin/rend iterate in reverse order"
,   "[color][iterators]"
)
{   generic_color<int, 4> c{1, 2, 3, 4};
...
TEST_CASE
(   "generic_color::crbegin/crend provide const reverse iteration"
,   "[color][iterators]"
)
...
TEST_CASE
(   "generic_color::empty reports whether there are any channels"
,   "[color][capacity]"
)
...
TEST_CASE
(   "generic_color::size and max_size report the number of channels"
,   "[color][capacity]"
)
{   generic_color<int, 4> c;
    REQUIRE(c.size() == 4);
...
TEST_CASE
(   "generic_color::fill assigns the same value to every channel"
,   "[color][operations]"
)
{   generic_color<int, 4> c{1, 2, 3, 4};
    c.fill(7);
...
TEST_CASE
(   "generic_color::swap exchanges the channels of two colors"
,   "[color][operations]"
)
...
TEST_CASE
(   "generic_color equality and inequality compare all channels"
,   "[color][comparisons]"
)
{   generic_color<int, 4> a{1, 2, 3, 4};
...
TEST_CASE
(   "generic_color relational operators use lexicographical order"
,   "[color][comparisons]"
)
{   generic_color<int, 4> a{1, 2, 3, 4};
    generic_color<int, 4> b{1, 2, 3, 4};
